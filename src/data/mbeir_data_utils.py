...
def build_mbeir_dataset_from_config(config, img_preprocess_fn, tokenizer, dataset_type, seq2seq_tokenizer=None):
    data_config = config.data_config
    if dataset_type == DatasetType.QUERY:
        # query_pool_dataset = MBEIRQueryDataset(
        #     mbeir_data_dir=config.mbeir_data_dir,
        #     query_data_path=data_config.query_data_path,
        #     img_preprocess_fn=img_preprocess_fn,
        # )
        # query_collator = MBEIRQueryPoolCollator(
        #     tokenizer=tokenizer,
        #     image_size=tuple(map(int, data_config.image_size.split(","))),
        # )

        query_data_path = data_config.query_data_path
        cand_pool_path = data_config.cand_pool_path
        mode = Mode.TRAIN

        query_pool_dataset = MBEIRMainDataset(
            mbeir_data_dir=config.mbeir_data_dir,
        ...
        return query_pool_dataset, query_collator
