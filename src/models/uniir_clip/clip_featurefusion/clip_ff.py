...
class CLIPFeatureFusion(nn.Module):
    ...
    def encode_combined_input(self, txt_tensor, img_tensor, txt_mask, img_mask):
        """Encode multimodal input using CLIP and T5 models
        ...
        img_feat = self.encode_image(img_tensor)
        # img_feat = img_feat * img_mask.unsqueeze(-1).unsqueeze(-1).expand_as(img_feat)
        combined_features = torch.cat([img_feat, txt_feat], dim=1)  # shape: [batch_size, seq_len, embed_dim]

        return combined_features  # shape: [batch_size, length, embed_dim]


        output = transformer_output.last_hidden_state

        # Pool the output of the T5 transformer to get the final features
        return output[:,0], output[:, 1:]  # shape: [batch_size, embed_dim]
