...
def create_index(config):
    ...
    for cand_pool_name in cand_pool_name_list:
        ...
        embed_data_path = os.path.join(genir_dir, embed_dir_name, expt_dir_name, split_name, embed_data_file)
...
def run_retrieval(config):
    ...
    for split_name in dataset_types:
        ...
        if retrieval_dataset_config and retrieval_dataset_config.enable_retrieve:
            ...
            dataset_embed_dir = os.path.join(genir_dir, embed_dir_name, expt_dir_name, split_name)
            splits.append(
                (
                    split_name,
                    dataset_embed_dir,
    ...
    eval_results = []
    cand_index_dir = os.path.join(genir_dir, index_dir_name, expt_dir_name, "cand_pool")
...
def run_hard_negative_mining(config):
    ...
    # Load query IDs
    dataset_embed_dir = os.path.join(genir_dir, embed_dir_name, expt_dir_name, dataset_split_name)
    ...
    cand_index_dir = os.path.join(genir_dir, index_dir_name, expt_dir_name, cand_pool_split_name)
...
def parse_arguments():
    ...
    parser.add_argument("--genir_dir", type=str, default="/data/GENIUS")
    parser.add_argument("--mbeir_data_dir", type=str, default="/data/GENIUS/mbeir_data")
