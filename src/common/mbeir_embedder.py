...
@torch.no_grad()
def generate_embeds_and_ids_for_dataset_with_gather(model, data_loader, device, use_fp16=True, is_query=False):
    embedding_tensors = []
    id_list = []
    ...
    for batch in data_loader:
        ...
        with autocast(enabled=use_fp16):
            embeddings_batched, ids_list_batched = model(batch, encode_mbeir_batch=True, is_query=is_query)
    ...
    for split_name, split_dir, dataset_name_list, cand_pool_name_list in splits:
        for dataset_name, cand_pool_name in zip(dataset_name_list, cand_pool_name_list):
            ...
            if not dist.is_initialized() or dist.get_rank() == 0:
                ...
                id_path = os.path.join(genir_dir, embed_dir_name, expt_dir_name, split_name, id_data_name)
...
def parse_arguments():
    ...
    parser.add_argument("--genir_dir", type=str, default="/data/UniIR")
